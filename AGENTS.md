...
- **Inherited vocabulary.** This repo was seeded from `wax` ([ADR-0001](docs/adr/0001-self-hosted-single-user-service.md)), and a rename reaches identifiers but not the prose around them — so seeded docs and comments can go on describing *wax's* domain long after every symbol they name is ours, where no identifier grep will ever find them. Prose adapted from the sibling project is restated in this project's domain language (canonical: [`docs/domain/README.md`](docs/domain/README.md)); a sentence that is only true of `wax` is a defect even when it reads cleanly. Citing `wax` as the origin of the stack is not what this is about — borrowed *meaning* is.
