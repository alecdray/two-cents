...
# Optional section arg:    ./scripts/check-docs-provider.sh [env|product|teller|plaid]
...
DOC_GLOBS=(docs README.md CONTEXT.md .env.template docker-compose.example.yml AGENTS.md Dockerfile)
...
# ---- section: product doc provider name ------------------------------------
check_product() {
  printf '== docs/product/README.md ==\n'
  if grep -Eq 'Plaid' docs/product/README.md; then ok "docs/product/README.md references Plaid"; else bad "docs/product/README.md does not reference Plaid"; fi
  if grep -Eqi 'teller' docs/product/README.md; then bad "docs/product/README.md still references Teller"; else ok "docs/product/README.md references no Teller"; fi
}
...
check_plaid() {
  ...
  assert_present 'access_token'              src/internal/plaid/AGENTS.md
  ...
  assert_present 'Plaid Link'                docs/product/README.md
  assert_present 'public_token'              src/internal/plaid/AGENTS.md
...
case "$section" in
  ...
  plaid)   check_plaid ;;
  teller)  check_teller ;;
  all)     check_env; check_product; check_plaid; check_teller ;;
  *) echo "unknown section: $section (use: env|product|plaid|teller|all)"; exit 2 ;;
esac
