...
- **Real backend, seed the database.** No `page.route(...)` / `page.unroute(...)` / `page.fulfill(...)`, no MSW/nock/sinon, no `jest.mock` / `vi.mock`. Need a specific data shape? Insert it into the SQLite DB directly via a `helpers/` seeder. There is no write API to drive — seeding is the only data-setup path.
