...
A personal finance app that pulls your own bank transactions and balances (via Teller) and makes spending legible: aggregation, budgeting, and month tracking. Self-hosted, single-user. *Your two cents on your own spending.*
...
Skeleton scaffolded — a hello-world page validates the full pipeline (templ → Tailwind → httpx → goose → Docker). Domain modules (`teller`, `accounts`, `transactions`, `categorization`, `budget`, `tracker`, `reporting`) are not yet built.
