...
- `sweep-numeric` — a numeric snapshot's section, holding `sweep-action` (the headline), `sweep-breakdown` (the supporting figures) and `sweep-timeline` (the dated derivation).
...
- `sweep-checking` / `-savings` / `-required` / `-safety-margin` — the individual supporting figures, each with a `-label` sibling. `sweep-required` is the peak of the timeline — what must stay in checking ([ADR-0024](../adr/0024-cash-flow-timeline-sweep.md)).
- `sweep-timeline` — the dated derivation the figure came from, holding one `sweep-timeline-row` per event, each with `-date`, `-label`, `-amount` and `-running`. `sweep-timeline-peak` marks the single row that set the figure, so an assertion about "which moment decided the number" has something to pin. `sweep-timeline-empty` replaces the table when nothing falls due in the window — a real result, not a failure, so a negative assertion on the table must name which case it means.
...
- `sweep-schedule` — the sweep schedule region (`ScheduleRegionID()`), rendered by the sweep's adapters even though `schedule` owns the data. It holds `schedule-add-form` (with `schedule-add-name` / `-amount` / `-day` / `-anchor` / `-direction` / `-cadence` and `schedule-add-submit`), `schedule-list` holding one `schedule-row` per declared item (each wrapping its edit form as `schedule-row-form`, with the same field testids under a `schedule-row-` prefix, plus `schedule-row-active`, `-save` and `-delete`), and `schedule-empty` in place of the list when nothing is declared. The shared field group roots at `schedule-add-fields` / `schedule-row-fields` — one component rendering both, so the add form and every row can never accept different things. Row field testids repeat once per row, as the rules list's do.
- `schedule-add-error` / `schedule-row-error` — the inline validation messages, scoped to the form that failed.
- The page root wraps two independently swappable regions: the snapshot region the Run now action targets (`SweepRegionID()`, rendered as `SweepSnapshotFrag`) and the schedule region every schedule mutation targets. Neither swap disturbs the other — a run leaves the schedule alone, and a schedule edit leaves the snapshot on screen, because a snapshot records what was advised at an instant.
...
- `wrap-spending` — the wrap's total-spending figure (a derived summary; not a drill — clicking it scrolls to the `wrap-all-transactions` section).
