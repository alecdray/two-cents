...
- `sweep-schedule` — the sweep schedule region (`ScheduleRegionID()`), rendered by the sweep's adapters even though `schedule` owns the data. It holds `schedule-add-form` (with `schedule-add-name` / `-amount` / `-day` / `-anchor` / `-direction` / `-cadence` and `schedule-add-submit`), one `schedule-row` per declared item (the same field testids under a `schedule-row-` prefix, plus `schedule-row-active`, `-save` and `-delete`), and `schedule-empty` when nothing is declared. Row field testids repeat once per row, as the rules list's do.
