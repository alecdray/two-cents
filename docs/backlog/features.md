...
- **Sync result count / last-synced time.** The in-flight signal shipped ([ADR-0015](../adr/0015-app-wide-request-feedback.md)); still deferred is a concrete "Synced (n updated)" count (needs a sync summary the service doesn't return today) and a surfaced last-synced time.
