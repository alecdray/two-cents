...
- **It issues the existing two writes, not a combined one.** Re-categorize and mark-transfer-destination stay separate writes the one modal invokes, so each keeps its own coupling validation (a Spending choice still requires a Category; a destination mark still writes only the transfer facet). The modal unifies the *presentation*, not the operations.
...
Rejected: per-view inline pickers (the duplication this removes, and unshareable across views without coupling the editor to each caller); a single combined save endpoint (loses the independent coupling validation each write enforces); whole-row-click to edit (collides with navigational rows). The drill's own reconciliation invariant is unchanged ([ADR-0009](0009-category-spend-drill-down.md)) — only how its rows are edited changes.
