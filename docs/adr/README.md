...
| [0011](0011-reusable-transaction-editing-modal.md) | Reusable transaction-editing modal | Transaction editing is one modal editor reused by every view that lists transactions, opened by an explicit Edit affordance — replacing the per-view inline pickers that otherwise drift. A domain-free modal shell (primitive) hosts transactions-owned editing content; on save it issues the existing two writes (each keeping its coupling validation) and emits the change event so callers self-refresh, keeping the editor context-free and shareable. |
