...
| **Transaction sync** | Cursor-based incremental sync on a ~6h cron + on demand; dedupe by provider id, pending→posted reconcile, `removed`-set deletes; fault-isolating, so one failing bank never stops the others | [transactions](../../src/internal/transactions/README.md); [ADR-0022](../adr/0022-fault-isolating-sync-pass.md) |
