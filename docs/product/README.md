...
|---|---|---|
| **Bank connections** | Connect / disconnect / reconnect banks via Plaid Link (or a deterministic fake selected by config); a connection the user must act on surfaces a needs-reconnect state, and one failing for any other reason surfaces as a stale balance — never silent either way | [accounts](../../src/internal/accounts/README.md), [plaid](../../src/internal/plaid/AGENTS.md); [ADR-0002](../adr/0002-bankprovider-abstraction.md), [ADR-0006](../adr/0006-bank-provider-selected-by-config.md), [ADR-0021](../adr/0021-fault-isolating-sync-pass.md) |
| **Transaction sync** | Cursor-based incremental sync on a ~6h cron + on demand; dedupe by provider id, pending→posted reconcile, `removed`-set deletes; fault-isolating, so one failing bank never stops the others | [transactions](../../src/internal/transactions/README.md); [ADR-0021](../adr/0021-fault-isolating-sync-pass.md) |
