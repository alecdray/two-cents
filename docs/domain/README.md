...
| **Net income** | Total Income − total Spending (Spending already net of refunds). A *flow*; a derived net, distinct from **gross income** (Σ the month's Income legs). It underpins **Surplus** (net income − savings) but is not itself a displayed wrap figure; gross income is the wrap's income drill-in target ([ADR-0012](../adr/0012-wrap-income-savings-and-month-list-drill-ins.md)), since net income has no single underlying transaction set. |
| **Surplus** | Net income − Savings contribution (i.e. income − spending − savings). A *flow*: the month's income left unallocated after both spending and funding savings — negative is a **deficit**. A **closed-month** figure shown on each wrap, not on the current-month tracker (mid-month it is a noisy running tally; the tracker is forward-looking — remaining and pace); a derived net, not a drill target. Distinct from **Net income** (subtracts spending only, so savings swept aside still counts as earned) and from the **Net cash** *position*. |
...
| **stale balance** | An Account whose balance has gone too long without a successful refresh, marked on the overview so an old figure is not read as a current one; an Account never synced counts as stale. It exists because a Connection failing on a provider error we cannot classify as user-actionable stays in its current state and says nothing, so **needs-reconnect** does not cover that case — the two are independent ([ADR-0021](../adr/0021-fault-isolating-sync-pass.md)). |
...
| **Stale balance** | An Account balance that has gone too long without a confirmed refresh ([ADR-0021](../adr/0021-fault-isolating-sync-pass.md)) — a fact `accounts` owns and defines. Distinct from an *unknown* balance (never reported) and from *needs-reconnect* (an actionable connection failure); a balance can be stale while its connection still looks healthy. |
...
### Cash sweep recommendation — forward-looking, *persisted*, navigable
