...
| **Surplus** | Net income − Savings contribution (i.e. income − spending − savings). A *flow*: the month's income left unallocated after both spending and funding savings — negative is a **deficit**. Shown on the current-month tracker and each wrap; a derived net, not a drill target. Distinct from **Net income** (subtracts spending only, so savings swept aside still counts as earned) and from the **Net cash** *position*. |
