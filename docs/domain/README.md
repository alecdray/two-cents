...
| **needs-attention** | A read-side **view/filter** on `/transactions` spanning every Transaction the system left for the user to resolve or refine — the union of three states: an unresolved inflow (Classification = **needs-review**), uncategorized **Spending** (Classification = Spending, no Category), or a **Transfer** whose destination is **unknown** (unpaired, so a possible Savings contribution is uncounted). A lens, **not** a Classification — `needs-review` remains the single Classification value among its members. The deep-link target for the home needs-attention alert (a later slice). |
