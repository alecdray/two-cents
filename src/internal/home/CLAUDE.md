...
- **Partial flag:** a month is partial when there is at least one connection and the month is at or before the earliest connection's month; with no connections nothing is partial. **Caveat:** this is a cheap single-connection approximation — with multiple connections added at different times it can **under-flag** a later connection's own backfill-edge months. A precise per-connection history window is deferred (see `docs/roadmap.md`).
