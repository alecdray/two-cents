...
- Budget-relative: unlike `reporting` (actuals-only), the Tracker compares actuals against the Budget config the composer passes in; with no budget it reports actuals only (`NeedsBudget`).
- **Surplus** ([glossary](../../../docs/domain/README.md)) is an actuals figure (income − total spend − savings), distinct from Total remaining, which uses the budget's income/savings *targets*. It is derived from the actuals and so is populated in both modes.
